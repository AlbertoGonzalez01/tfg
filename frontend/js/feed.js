...
async function cargarFotos() {
  const userId = localStorage.getItem('userId');
  if (!userId) return alert('No has iniciado sesión');

  try {
    const res = await fetch(`https://backend-tfg-9u97.onrender.com/api/photos/feed/${userId}`);
    ...
    fotos.forEach(foto => {
      ...
      const nombreUsuario = foto.userId?.user || 'Desconocido';

      div.innerHTML = `
        <p><strong>${nombreUsuario}</strong> subió:</p>
